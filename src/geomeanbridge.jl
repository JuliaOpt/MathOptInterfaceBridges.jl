...
function GeoMeanBridge{T}(instance, f::MOI.VectorAffineFunction{T}, s::MOI.GeometricMeanCone) where T
    d = s.dimension
    n = d-1
    l = ilog2(n)
    N = 1 << l
    xij = MOI.addvariables!(instance, N-1)
    ...
    for i in 1:l
        ...
        for j in 1:(1 << (i-1))
            ...
            socrc[offset + j] = MOI.addconstraint!(instance, moivcat(a, b, c), MOI.RotatedSecondOrderCone(3))
        end
        offset = offsetnext
    end
...
function MOI.delete!(instance::MOI.AbstractInstance, c::GeoMeanBridge)
    MOI.delete!(instance, c.xij)
    MOI.delete!(instance, c.tubc)
...
function _getconstrattr(instance, a, c::GeoMeanBridge{T}) where T
    ...
    for i in 1:(c.d-1)
        j = ((i-1) >> 1) + 1
        k = i - 2(j - 1)
        output[1+i] = MOI.get(instance, a, c.socrc[offset+j])[k]
    end
    output
end
function MOI.get(instance::MOI.AbstractInstance, a::MOI.ConstraintPrimal, c::GeoMeanBridge)
    output = _getconstrattr(instance, a, c)
    N = length(c.xij)+1
    # the constraint is t - x_l1/sqrt(2^l) ≤ 0, we need to add the value of x_l1
    output[1] += MOI.get(instance, MOI.VariablePrimal(), c.xij[1]) / sqrt(N)
    output
end
MOI.canget(instance::MOI.AbstractInstance, a::MOI.ConstraintDual, c::GeoMeanBridge) = false
#function MOI.get(instance::MOI.AbstractInstance, a::MOI.ConstraintDual, c::GeoMeanBridge)
#    output = _getconstrattr(instance, a, c)
#end

# Constraints
MOI.canmodifyconstraint(instance::MOI.AbstractInstance, c::GeoMeanBridge, change) = false
