...
macro model(modelname, ss, sst, vs, vst, sf, sft, vf, vft)
    ...
    code = quote
        function MathOptInterfaceUtilities.broadcastcall(f::Function, model::$modelname)
            $(Expr(:block, _broadcastfield.(:(MathOptInterfaceUtilities.broadcastcall), funs)...))
        end
        function MathOptInterfaceUtilities.broadcastvcat(f::Function, model::$modelname)
    ...
    for (cname, sets) in ((scname, scalarsets), (vcname, vectorsets))
        code = quote
            $code
            function MathOptInterfaceUtilities.broadcastcall(f::Function, model::$cname)
                $(Expr(:block, _callfield.(:f, sets)...))
            end
            function MathOptInterfaceUtilities.broadcastvcat(f::Function, model::$cname)
