...
@testset "Basic solve, query, resolve" begin
    ...
    @test MOI.canget(m, MOI.ConstraintSet(), vc1)
    s = MOI.get(m, MOI.ConstraintSet(), vc1)
    @test s == MOI.GreaterThan(0.0)

    @test MOI.canget(m, MOI.ConstraintSet(), vc2)
    ...
    @test v[3] == z
    @test cf.variables == v
    @test MOI.get(m, MOI.ConstraintFunction(), c).variables == [v[1], v[2]]
    @test MOI.get(m, MOI.ObjectiveFunction()).variables == [v[1], v[2]]
...
@testset "Quadratic functions" begin
    ...
    @test !(MOI.get(m, MOI.ConstraintFunction(), c1) ≈ f3)
    ...
    @test MOI.get(m, MOI.NumberOfConstraints{MOI.VectorAffineFunction{Int},MOI.SecondOrderCone}()) == 1
    ...
    @test MOI.get(m, MOI.NumberOfConstraints{MOI.VectorAffineFunction{Int},MOI.SecondOrderCone}()) == 2
    ...
    loc1 = MOI.get(m, MOI.ListOfConstraints())
    ...
    @test MOI.get(m, MOI.NumberOfConstraints{MOI.VectorAffineFunction{Int},MOI.SecondOrderCone}()) == 1
    @test MOI.get(m, MOI.ConstraintFunction(), c6).constant == f6.constant
    ...
    f =  MOI.get(m, MOI.ConstraintFunction(), c6)
    @test f.outputindex == [1]
    @test f.variables == [x]
    @test f.coefficients == [2]
    @test f.constant == [6, 8]

    f =  MOI.get(m, MOI.ConstraintFunction(), c7)
    @test f.variables == [x]

    s =  MOI.get(m, MOI.ConstraintSet(), c7)
